...
这是一份在持续改进的文档。它已经是一个开源（代码）项目，将要发布的版本是0.8。复制，使用，修改或是基于当前文档的新的工作，都是MIT类型的许可。对本文档做出贡献的人需要同意贡献者的许可。更多内容参考文件[LICENSE](LICENSE)。

我们希望这个项目能够使大家容易使用，复制，修改并且以此为基础做出新的东西，希望有一些创造性的内容。

改进性的评论和建议是欢迎的。我们计划修改并扩展这个文档，使之成为一个可以使用的库。当评论的时候，请注意[the introduction](#S-introduction)描述了我们的目标和一般方法。
贡献者的名单在[这](#SS-ack)。


问题:
* 这些规则还没有被完整的验证完整性，一致性或者强制性。
* 三个问号（？？？）标记已知的遗失信息。
* 更新参考信息一节；许多C++11之前的代码可能太老了。
* 对于或多或少的更新到最新的to-do列表，请参考[To-do: Unclassified proto-rules](#S-unclassified)

你可以[read an explanation of the scope and structure of this Guide](#S-abstract)或者直接跳到


* [In: 简介](#S-introduction)
* [P: 哲学](#S-philosophy)
* [I: 接口](#S-interfaces)
* [F: 函数](#S-functions)
* [C: 类和继承](#S-class)
...
* [NL: 命名和布局规则](#S-naming)
* [FAQ: 常见问题的答案](#S-faq)
* [Appendix A: 库](#S-libraries)
* [Appendix B: 现代化代码](#S-modernizing)
* [Appendix C: 讨论](#S-discussion)
* [Appendix D: 支持的工具](#S-tools)
* [术语](#S-glossary)
* [To-do: 未归类的规则原型](#S-unclassified)
...
[最小化 `try`范围](#Re-catch) --
[如果没有异常？](#Re-no-throw-codes)
* `for`:
[基于范围的for循环和for循环](#Res-for-range) --
[for循环和while循环](#Res-for-while) --
[for初始化](#Res-for-init) --
[空循环](#Res-empty) --
...
本文档是为了良好的使用C++的一系列的指南。其目的是帮助大家更有效的使用现代C++。
现代C++意味着高效的使用ISO C++标准（当前标准是C++17，大部分的建议也适用于C++14和C++11）。
换句话说，如果你从现在开始写代码，想让你的代码看起来像是经历了5年还是经历了10年？

该指南致力于相对比较高层的问题，像是接口，资源管理，内存管理和并发。
这些规则会影响应用架构和库的设计。
使用这些规则，可以使得你的代码静态类型安全，没有资源泄露，能够捕捉比你今天更多的编程逻辑错误。并且运行的更快，你可以把事情做的更好。

我们不是很关心比较低层次的问题，像是命名转换和缩进风格。
然而，**没有什么规则可以帮助程序员跨越这些**。

我们最初的规则强调安全性（多种多样的）和简单性。
他们可能太严格了。
我们期望介绍更多的特例来更好的适应现实世界的需求。

你会发现有些规则跟你的期望相反，甚至与你的经验相反。如果我们还没有以任何方式改变你的代码风格，我们就失败了。请试着验证或者反驳这些规则。特别的，我们真的希望一些规则能够被衡量或者有更好的例子。

你可能会发现一些规则是显而易见的或者是微不足道的。请记住，指南的一个目标是帮助一些经验比较欠缺的人或者不同语言背景的人能够赶上进度。

其中的一些规则被设计来支持一些分析工具。违反规则会被标记到对应的规则的参考（或者链接）。我们并不期望你在写代码之前能够背下所有的规则。**可以将这些指导原则看作是可供人类阅读的工具规范。**

这些规则将逐步的引入到代码库中。我们计划根据这些规则来创建工具，并希望其他人也会这么做。

欢迎提出改进意见和建议。我们计划修改和扩展这个文档，使之成为一系列提高理解和改善语言的库。

# <a name="S-introduction"></a>In: 简介

这是一系列对于现代C++（当前是C++17）的核心指南，可以将其看作为特性增强和ISO 技术规范。目的是帮助C++程序员写出更简单，更高效和可维护性更高的代码。
...
* [In.aims: 目标](#SS-aims)
* [In.not: 非目标](#SS-non)
* [In.force: 加强](#SS-force)
* [In.struct: 文档结构](#SS-struct)
* [In.sec: 主要章节](#SS-sec)

## <a name="SS-readers"></a>In.target: 目标读者

所有C++程序员。包括可能[考虑使用C的程序员](#S-cpl)。

## <a name="SS-aims"></a>In.aims: 目标

本文档的目标是帮助开发者采用现代C++（当前是C++），并且在不同的代码库中有统一的风格。

我们不会认为这些规则可以应用到每一个代码库。升级老的系统是很困难的。然而，我们相信一个使用这些规则的程序会比那些不适用的，会有更少的错误和更易于维护。规则也会经常导致更快的/更简单的初始开发。
至于我们能够表述的，这些规则导致代码的性能与较老的、更传统的技术相同或更好；他们遵从零开销原则（“你不会对于你不使用的内容进行支付”或者“当你正确的使用抽象机制，你至少可以得到跟你使用低层语言所得到的性能”）。考虑
...
花点时间理解指导原则对程序的影响。
指导原则依据“父集的子集”的原则来设计([Stroustrup05](#Stroustrup05))。
他们不仅仅是简单的定义了一个C++的子集来使用（为了可靠性，安全性，性能，或者其他的要求）。
相反的，强烈建议来使用一些简单的“扩展”([library components](#S-gsl))
Instead, they strongly recommend the use of a few simple "extensions" ([library components](#S-gsl))
that make the use of the most error-prone features of C++ redundant, so that they can be banned (in our set of rules).

这些规则强调静态类型安全和资源安全。
基于这个原因，他们强调范围检查的可能性，避免解引用`nullptr`，避免悬挂指针，和异常的系统使用（通过RAII）。
...
许多规则是约定俗成的。
我们认为那些只是简单的告诉你不要这么做。却不给出对应的解决方法的规则是令人不舒服的。
其结果是，有些规则只能由启发式支持，而不能由精确的和机械的可验证的检验来支持。
其他规则阐明了一般原则。对于那些更一般的规则，更详细和更具体的规则提供部分的检查。

这些指导方针讨论了C++的核心和它的使用。
我们希望大多数大的组织，特定应用领域，甚至是大的项目需要更深入的规则，可能更深入的限制和更深入的库支持。
例如，硬实时程序员通常不能自由地使用自由存储(动态内存)，并且在库的选择上受到限制
...
这些规则被设计来允许[逐步的采用](#S-modernizing)。
一些规则增加了一些形式的安全性，还有一些目的是减少问题发生的可能性，还有一些两者都有。某些规则是用来阻止问题的，但是会经常的导致合法的C++被禁止。然而，当有两种方法来实现一个想法的时候，我们更希望使用那个可以显示错误的常见来源的那一个方法。


## <a name="SS-non"></a>In.not: 非目标

这些规则不是最小的或者是正交的。
特别的，一般规则应该是简单的，不应该是强制的。
而且，通常很难理解一般规则的含义。比较有针对性的规则，通常是比较简单和强制的，但是，没有通用规则的话，它们只能是特殊例子的一个长列表。我们给出的规则旨在既帮助新手，又可以支持专家使用。一些规则是完全强制的，而另外的一些是启发性的。

这些规则不要像一本书一样连续阅读。你可以使用链接来浏览一下。然而，它们的主要用途还是为了作为工具。也就是说，工具查找违规行为，并返回违规规则的链接。然后规则提供原因，这么做所带来的后果的例子，和建议的改善方法。
这些指南并不是为了作为C++教程的替代品。如果你需要某种等级的教程，请参考[这](#S-references)。

这不是一个来交给你如何从旧的C++代码转向现代C++代码的指南。它旨在以具体的方式阐明新代码的思想。然而，在[现代化章节](#S-modernizing)，有一些可以使用的方法来使得代码现代化，使代码变大有活力或者升级代码。重要的是，规则支持逐步的采用：一次性的完全转换大型代码是不可行的。
...
## <a name="SS-force"></a>In.force: 强制性

对于大型的代码库，没有强制性是很难管理的。
强制施行所有的规则只有在规则的数量比较少或者特定的用户社区才是可能实现的。

* 但是我们想要很多的规则，并且所有人都能使用这些规则。
* 但是不同的人有不同的需求。
* 但是大家并不喜欢阅读很多的规则。
* 但是大家记不住很多规则。
...
我们想要这份指南能够帮助很多的人，使代码能够保持一致性，强烈建议大家现代化自己的代码。
我们鼓励最佳实践，而不是把一切都留给个人选择和管理压力。理想的情况下是使用所有的规则，这样可以带来最大的价值。

这就造成了相当多的困境。
我们试着通过使用工具来解决这些问题。
每一个规则都有一个强制的部分，并列出了强制的一系列想法。
强制可以通过代码审查，静态代码分析，编译或者运行时检查。
只要有可能，我们更希望机械检查（人工是比较慢的，不确切的，并且很容易厌烦的）和静态检查。
运行时检查建议只在没有其他可用的方法的时候使用；我们不想引入“distributed bloat”。
在适当的地方，我们用相关规则组的名称(称为“概要文件”)来标记规则(在强制章节中)。
一个规则可以是几个概要文件的一部分，也可以不属于任何概要文件。
在一开始，我们仅对公共需求有一些摘要文件（理想）：

* **类型**：没有类型冲突（使用显示转换，联合体或者不定参数将类型`T`重新解释为`U`）
* **边界**：没有边界冲突（访问数组范围以外的元素）
* **生命周期**：没有泄露（`delete`失败或者多次`delete`），无法访问无效的对象（解参考`nullptr`，使用悬挂的引用）。

摘要文件是给工具来使用的，但是也可以给程序员来阅读。
...
## <a name="SS-struct"></a>In.struct: 文档的结构

每一个规则（指南，建议）可以有以下几部分：
Each rule (guideline, suggestion) can have several parts:

* 规则自己--例如，**不要使用裸`new`**
* 规则引用数--例如，**C.7**（跟类有关的第7条规则）
  因为主要章节并不是有序的，我们使用字母作为规则引用数的第一部分。
  当我们添加或删除规则时，我们会在编码中留下空白，以最大限度地减少“干扰”。
* **原因**（）
...
每一节（例如，“P”表示“哲学”）和每一个子节（例如，“C.hire”表示“类继承（面向对象编程）”）都有一个缩写，为了方便查找和引用。
主章节的缩写还被用来表示规则编号（例如，“C.11”表示“使具体类型成为规则”）。
...
* [P.1: 直接在代码中表达想法](#Rp-direct)
* [P.2: 用标准C++编写代码](#Rp-Cplusplus)
* [P.3: 表达意图](#Rp-what)
* [P.4: 在理想情况下，一个程序应该是静态类型安全的](#Rp-typesafe)
* [P.5: 编译期检查好于运行期检查](#Rp-compile-time)
* [P.6: 无法在编译期检查的，应该是运行期检查](#Rp-run-time)
* [P.7: 在早期捕捉运行时错误](#Rp-early)
* [P.8: 不要泄漏任何资源](#Rp-leak)
* [P.9: 不要浪费时间和空间](#Rp-waste)
* [P.10: 不变的数据优于可变的数据](#Rp-mutable)
* [P.11: 封装混乱的构造，而不是分散在代码中](#Rp-library)
* [P.12: 适当的支持工具](#Rp-tools)
* [P.13: 适当的支持库](#Rp-lib)

哲学性的规则通常是不能够机械化检查的。  
然而，个别反应这些哲学主题的规则是可以机械化检查的。
没有哲学基础，具体/特定/可检查的规则缺乏原理。

### <a name="Rp-direct"></a>P.1: 直接在代码中表达想法

##### 原因

编译器不会读取注释（或是设计文档），许多程序员也不会这么做（一致性）。
代码所表达的内容有清晰的语义，能够（在原则上）被编译器和其他的工具检查。
...
```
函数`month`的第一个声明显示地返回一个`Month`类型，并且不会改变`Date`的状态。
第二个版本让读者猜测，可能会导致很多无法捕捉的错误。
The first declaration of `month` is explicit about returning a `Month` and about not modifying the state of the `Date` object.
The second version leaves the reader guessing and opens more possibilities for uncaught bugs.

##### 反面代码示例

这个循环是`std::find`的受限制的形式：
...
```
拥有良好设计的库比直接使用语言特性能够更好的表达意图（表达什么将要被实现，而不是什么怎么是在被做的）。

一个C++程序员应该知道标准库的基础，并且在适当的地方使用。
任何程序员都应该熟悉当前所工作的项目的基本库，并且正确的使用它们。
任何使用这些指南的程序员应该知道[guidelines support library](#S-gsl)，并且正确的使用它。
...
```cpp
    ...
    change_speed(2.3);
```
一个好的方法应该显示的指出double的含义（新的速度或者旧速度的增量）和使用物理单位：
...
* 始终如一地使用`const`（检查一个成员函数是否修改它们的对象；检查函数是否修改使用指针或者引用传入的参数）
...
### <a name="Rp-Cplusplus"></a>P.2: 用标准C++编写代码
